// API key
const API_KEY = "1d2ef6523880e51267e17bd7a8297b8d5090cbaa1f944bb3840d54fc391a1dea";
...
// Shows a loadig spinner and message 
...
// Generic function to make authentication API requests ro VirusTotal
...
async function scanURL() {
    ...
    try {
        ...
        showLoading("Getting scan result...");
...
// Handle the process of scanning a file using VirusTotal
async function scanFile() {
    const file = getElement('inputFile').files[0];
...
// Polls VirusTotal for analysis results, retrying untils complete or timeout
async function pollAnalysisResults(analysisId, fileName = '') {
    ...
    while(attempts < maxAttempts) {
        try {
            showLoading(`Analysis${fileName ? ` ${fileName} ` : ''}... (${((maxAttempts - attempts) * interval / 1000).toFixed(0)}s remaining)`);
